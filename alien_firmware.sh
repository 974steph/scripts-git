...
#
# ZIP File
# curl -sL "http://www.smoktech.com/support/upgrade/toolsandfirmware/ispalien" | xmllint --nowarning --format --html - 2>/dev/null | grep -i \.zip | sed 's/.*href="\(.*\.zip\).*/\1/'
#
# Name
# curl -sL "http://www.smoktech.com/support/upgrade/toolsandfirmware/ispalien" | xmllint --nowarning --format --html - 2>/dev/null | grep -i \.zip | sed 's/<[^>]\+>/ /g;s/ \+//'
#
# Version
# curl -sL "http://www.smoktech.com/support/upgrade/toolsandfirmware/ispalien" | xmllint --nowarning --format --html - 2>/dev/null | grep -i \.zip | sed 's/<[^>]\+>/ /g;s/ \+//;s/)//g' | cut -d '(' -f2
#
# Date:
...
#ZIP_FILE=$(curl -sL "http://www.smoktech.com/support/upgrade/toolsandfirmware/ispalien" | xmllint --nowarning --format --html - 2>/dev/null | grep -i \.zip | sed 's/.*href="\(.*\.zip\).*/\1/')
ZIP_FILE=$(xmllint --nowarning --format --html ${TMP_FILE} 2>/dev/null | grep -i \.zip | sed 's/.*href="\(.*\.zip\).*/\1/')
